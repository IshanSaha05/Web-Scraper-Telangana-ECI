...
func GetLink_ByID(constituency_id string) (string, string, error) {
	url := config.DefaultScrapeSite
	response, err := GetSite(url)
	...
	if found {
		fmt.Println("Link: ", fmt.Sprintf("https://results.eci.gov.in/AcResultGenDecNew2023/candidateswise-S29%s.htm", id))
		return fmt.Sprintf("https://results.eci.gov.in/AcResultGenDecNew2023/candidateswise-S29%s.htm", id), name, nil
	}
...
func ParseInsertDBPrintResults(response *http.Response, mongoDBObject *mongodb.MongoDB) {
	// Parsing the results.
	results := parseResults(response)

	// Inserting results into DB.
	err := mongoDBObject.InsertIntoDB(results)
	if err != nil {
		fmt.Println("Error while inserting documents into collections.")
		fmt.Println("Error: ", err)
		os.Exit(1)
	}
